    ...
    <section className="relative h-screen w-full overflow-hidden flex items-center justify-center px-4">
      ...
      <div className="absolute inset-0">
        <div 
          ...
          aria-hidden="true"
        />
        <div className="absolute inset-0 bg-[#2D2D2D]/30 backdrop-blur-[2px]" />
      </div>

      {/* Glass content card */}
      <motion.div 
        className="relative p-10 rounded-2xl bg-[#2D2D2D]/60 backdrop-blur-md border border-[#ffffff]/20 shadow-xl max-w-4xl w-full mx-4"
      ...
      >
        ...
        <motion.p
          className="text-lg md:text-xl text-[#BCBCBC] mb-10 max-w-2xl mx-auto"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.4, duration: 0.8 }}
        >
          Water, Energy, Hydrogen – Building Resilient Infrastructure for South Africa
        </motion.p>
      ...
      <motion.a
        href="#services"
        className="absolute bottom-8 left-1/2 transform -translate-x-1/2 p-3 rounded-full bg-[#2D2D2D]/50 backdrop-blur-sm border border-[#ffffff]/20 cursor-pointer z-10"
        animate={{ 
          y: [0, 10, 0],
          opacity: [0.8, 1, 0.8]
        }}
